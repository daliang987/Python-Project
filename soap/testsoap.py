...
def SoapServiceTest(url_str,service,reqstr):
    try:
        ...
        port=url.port
        conn=httplib.HTTPConnection(host,port,timeout=5)
        conn.request('POST','/wcm/services/%s' % service,reqstr,headers={'SOAPAction':'""'}) 
        resp=conn.getresponse()
        # print resp.status
...
if __name__ == "__main__":
    ...
    output=""
    all_url=[]
    msg="python testsoap.py -u http://192.168.51.63:8080/wcm -o a.txt"
    ...
    try:
        for url_str in all_url:
            ...
            for service in services:
                print 
                print '%s://%s:%i/wcm/services/%s' % (url.scheme,url.hostname,url.port,service)
                print '-'*45
                if output:
                    outputtxt(output,'\n%s://%s:%i/wcm/services/%s\n' % (url.scheme,url.hostname,url.port,service))
                    outputtxt(output,'-'*45+'\n')
